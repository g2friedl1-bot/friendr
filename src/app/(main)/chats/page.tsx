        ...
        <div className="space-y-1">
          {chats.map((chat) => (
            <Link key={chat.href} href={chat.href}
              className="flex items-center gap-4 px-4 py-4 rounded-2xl hover:bg-zinc-900 transition-all">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between mb-0.5">
                  ...
                  {chat.time && <span className="text-xs text-zinc-600">{chat.time}</span>}
                </div>
                <p className="text-zinc-500 text-sm truncate">{chat.lastText}</p>
              </div>
            </Link>
          ))}
        </div>
