          ...
          {users.map((user) => (
            <div key={user.id} className="flex items-center gap-4 px-4 py-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
              ...
              <button onClick={() => handleChat(user.id)}
                className="flex-shrink-0 w-9 h-9 rounded-full bg-zinc-100 dark:bg-zinc-800 hover:bg-brand flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-white transition-all">
