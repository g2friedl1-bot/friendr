  ...
  return (
    <main className="min-h-screen bg-white dark:bg-zinc-950 pb-24">
      <div className="max-w-lg mx-auto px-4 pt-8">
        ...
        {username && (
          <p className="text-zinc-500 dark:text-zinc-500 text-sm mb-6">Hey, {username} 👋</p>
        )}

        <div className="space-y-2">
          {posts.map((post, i) => (
            <div key={i} className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2 mb-1">
                    <Link href={`/users/${post.user.id}`} className="font-semibold text-zinc-900 dark:text-white hover:text-brand transition-colors text-sm">
                      {post.user.name}
                    </Link>
                    <span className="text-xs text-zinc-400 dark:text-zinc-600">{timeLabel(post.hoursAgo)}</span>
                  </div>
