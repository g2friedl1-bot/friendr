...
function generateResult(
  ...
  const introScore = count("introvert");
  const extroScore = count("extrovert");
  const brightScore = count("bright");
  const deepScore = count("deep");
  const empatheticScore = count("empathetic");
  const calmScore = count("calm");
  const spontaneousScore = count("spontaneous");

  const isIntrovert = introScore > extroScore;
  const isBright = brightScore >= 2;
  const isDeep = deepScore >= 2;
  const isEmpathetic = empatheticScore >= 2;
  const isCalm = calmScore >= 2;
  const isSpontaneous = spontaneousScore >= 2;
  ...
  const tags: string[] = [];
  if (isIntrovert) tags.push("Introvert");
  else tags.push("Extrovert");
  ...
  if (isOpenAll) {
    return {
      emoji: "🌍", title: "The Open Soul", tags,
      blurb: `You're a rare breed — someone genuinely open to everything life throws at you. ${isIntrovert ? "You recharge in your own world but shine when it counts." : "Your energy is infectious and you light up every room."} You're searching for a ${companionLabel} who matches your curiosity and endless capacity for connection.`,
    };
  }

  if (isBright && !isIntrovert) {
    return {
      emoji: "✨", title: "The Social Spark", tags,
      blurb: `You walk into a room and people notice. Your energy is magnetic and your laugh is contagious. ${isDeep ? "But beneath the brightness, you crave real depth too." : "You live for the moment and love bringing people together."} Your ideal ${companionLabel} can keep up with your vibe and isn't afraid to match your energy.`,
    };
  }

  if (isDeep && isIntrovert) {
    return {
      emoji: "🧠", title: "The Deep Thinker", tags,
      blurb: `Small talk bores you. You want the real stuff — ideas, feelings, the meaning behind things. You're ${isCalm ? "calm and observant" : "intense and passionate"}, and you form deep bonds with the few people who really get you. Your ideal ${companionLabel} won't just scratch the surface.`,
    };
  }

  if (isEmpathetic) {
    return {
      emoji: "💜", title: "The Heart of the Group", tags,
      blurb: `People come to you because you actually listen. You have an uncanny ability to make others feel seen and understood. ${isIntrovert ? "You're selective with your energy, but fiercely loyal to those you let in." : "Your warmth draws people in naturally."} You need a ${companionLabel} who appreciates emotional depth and reciprocates your care.`,
    };
  }

  if (isCalm && isSpontaneous) {
    return {
      emoji: "🌊", title: "The Free Spirit", tags,
      blurb: `You go with the flow — but intentionally. You don't stress the small stuff and you're always down for wherever the night takes you. ${isIntrovert ? "You recharge alone but adventure solo or with the right person." : "You're easygoing and effortlessly fun to be around."} Your ${companionLabel} should be just as laid-back and up for anything.`,
    };
  }

  if (!isIntrovert && hasInterest("travel", "hiking", "camping", "outdoors", "surfing", "skiing")) {
    return {
      emoji: "⚡", title: "The Adventurer", tags,
      blurb: `You live for experiences. New places, new people, new challenges — that's your fuel. ${isSpontaneous ? "You rarely plan and you love it that way." : "You love exploring with purpose and passion."} You need a ${companionLabel} who won't hold you back and will actually want to come along for the ride.`,
    };
  }

  if (hasInterest("art", "music", "writing", "painting", "drawing", "film", "photography")) {
    return {
      emoji: "🎨", title: "The Creative Soul", tags,
      blurb: `You see the world differently — and that's your gift. You turn feelings into something tangible and find beauty where others don't look. ${isIntrovert ? "You do your best work in solitude but open up beautifully with the right person." : "Your creativity comes alive with people around you."} Your ${companionLabel} should appreciate the art in everyday life.`,
    };
  }

  return {
    emoji: "🌟", title: "The Unique One", tags,
...
export default function QuizPage() {
  ...
  function toggleInterest(interest: string) {
    if (interest === "Anything & Everything") {
      setSelected((prev) =>
        prev.has("Anything & Everything") ? new Set() : new Set(ALL_INTERESTS)
      );
    } else {
      setSelected((prev) => {
  ...
  function selectAnswer(questionId: string, traits: string[]) {
    setAnswers((prev) => ({ ...prev, [questionId]: traits }));
  }
  ...
  const progress =
    step === "companion" ? 5 :
    step === "questions" ? 10 + (currentQ / QUESTIONS.length) * 60 :
    step === "interests" ? 80 :
    100;
  ...
  if (step === "results" && result) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-violet-950 via-fuchsia-950 to-rose-950 pb-32">
        <div className="max-w-lg mx-auto px-4 pt-8">
          <div className="flex items-center gap-3 mb-8">
            <Link href="/profile" className="w-9 h-9 flex items-center justify-center rounded-full bg-violet-800/60 hover:bg-violet-700/70 text-violet-200 hover:text-white transition-all">
          ...
          {/* Result card */}
          <div className="rounded-2xl bg-gradient-to-br from-fuchsia-500/20 to-rose-500/20 border border-fuchsia-500/40 p-6 mb-5 text-center">
            <div className="text-6xl mb-4">{result.emoji}</div>
            <h2 className="text-2xl font-bold text-white mb-4">{result.title}</h2>
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              {result.tags.map((tag) => (
                <span key={tag} className="px-3 py-1 rounded-full bg-fuchsia-500/30 border border-fuchsia-500/50 text-fuchsia-200 text-xs font-semibold">
                  {tag}
                </span>
              ))}
            </div>
            <p className="text-violet-200/90 leading-relaxed text-sm">{result.blurb}</p>
          </div>

          {/* Interests summary */}
          <div className="rounded-xl bg-violet-900/40 border border-violet-700/50 p-4 mb-4">
            <p className="text-xs font-semibold text-violet-300 uppercase tracking-wider mb-3">Your Interests ({selected.size})</p>
            <div className="flex flex-wrap gap-2">
              {[...selected].map((s) => (
                <span key={s} className="px-3 py-1 rounded-full bg-violet-800/60 border border-violet-600/50 text-violet-200 text-xs font-medium">{s}</span>
              ))}
            </div>
          </div>

          {/* Companion */}
          <div className="rounded-xl bg-violet-900/40 border border-violet-700/50 px-4 py-3 mb-8 flex items-center justify-between">
            <span className="text-violet-300 text-sm">Looking for a companion who is...</span>
            <span className="text-white font-semibold text-sm capitalize">{companion === "any" ? "Any / Open" : companion}</span>
          </div>

          <button onClick={handleRetake} className="flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-violet-600/60 text-violet-300 hover:bg-violet-800/40 hover:text-white font-semibold transition-all">
        ...
        <div className="max-w-lg mx-auto px-4 pt-8">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-violet-400">Almost done!</span>
              <span className="text-xs text-violet-400">{Math.round(progress)}%</span>
            </div>
            <div className="h-1.5 rounded-full bg-violet-800/60">
              <div className="h-full rounded-full bg-gradient-to-r from-fuchsia-500 to-rose-500 transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>
          </div>

          <div className="flex items-center gap-3 mb-6">
            <button onClick={() => { setCurrentQ(QUESTIONS.length - 1); setStep("questions"); }} className="w-9 h-9 flex items-center justify-center rounded-full bg-violet-800/60 hover:bg-violet-700/70 text-violet-200 hover:text-white transition-all">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <div>
              <h1 className="text-2xl font-bold text-white">Your Interests</h1>
              <p className="text-violet-300/80 text-sm">Pick everything that excites you</p>
            </div>
          </div>
          ...
          <div className="relative mb-4">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search interests..."
              className="w-full pl-10 pr-10 py-3 rounded-xl bg-violet-900/50 border border-violet-700/50 text-white placeholder-violet-400/60 focus:outline-none focus:border-fuchsia-500 transition-colors" />
            {search && (
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {[...selected].map((s) => (
                <span key={s} onClick={() => toggleInterest(s)}
                  className="flex items-center gap-1 px-3 py-1 rounded-full bg-fuchsia-500/30 border border-fuchsia-500/60 text-fuchsia-200 text-xs font-medium cursor-pointer hover:bg-fuchsia-500/50 transition-all">
          ...
          <div className="grid grid-cols-2 gap-2">
            {filtered.map((interest) => {
              ...
              return (
                <button key={interest} onClick={() => toggleInterest(interest)}
                  className={`flex items-center justify-between px-4 py-3 rounded-xl border text-sm font-medium text-left transition-all ${isSelected ? "bg-fuchsia-500/25 border-fuchsia-500/70 text-white" : "bg-violet-900/40 border-violet-700/50 text-violet-200 hover:border-fuchsia-500/50 hover:text-white"}`}>
                  <span>{interest}</span>
            ...
            {filtered.length === 0 && (
              <div className="col-span-2 text-center py-8 text-violet-400">No interests match &ldquo;{search}&rdquo;</div>
            )}
          </div>
        </div>

        {/* Sticky submit bar — sits above the bottom nav */}
        <div className="fixed bottom-[65px] left-0 right-0 bg-violet-950/95 backdrop-blur-md border-t border-violet-800/50 px-4 py-4 z-40">
          <div className="max-w-lg mx-auto flex items-center justify-between gap-4">
            ...
            <button onClick={handleSubmit} disabled={selected.size === 0}
              className="flex items-center gap-2 px-6 py-2.5 rounded-full bg-gradient-to-r from-fuchsia-500 to-rose-500 text-white font-semibold text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:scale-105 transition-all shadow-lg shadow-fuchsia-500/30">
              Next
    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-violet-950 via-fuchsia-950 to-rose-950 pb-24">
        <div className="max-w-lg mx-auto px-4 pt-8">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-violet-400">Question {currentQ + 1} of {QUESTIONS.length}</span>
              <span className="text-xs text-violet-400">{Math.round(progress)}%</span>
            </div>
            <div className="h-1.5 rounded-full bg-violet-800/60">
              <div className="h-full rounded-full bg-gradient-to-r from-fuchsia-500 to-rose-500 transition-all duration-500" style={{ width: `${progress}%` }} />
          ...
          <div className="flex items-center gap-3 mb-8">
            <button
              onClick={() => {
                if (currentQ === 0) setStep("companion");
                else setCurrentQ((q) => q - 1);
              }}
              className="w-9 h-9 flex items-center justify-center rounded-full bg-violet-800/60 hover:bg-violet-700/70 text-violet-200 hover:text-white transition-all"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
          ...
          <div className="space-y-3 mb-8">
            {q.options.map((opt) => {
              ...
              return (
                <button
                  key={opt.label}
                  onClick={() => selectAnswer(q.id, opt.traits)}
                  className={`w-full text-left px-5 py-4 rounded-xl border text-sm font-medium transition-all ${
                    isSelected
                      ? "bg-fuchsia-500/25 border-fuchsia-500/70 text-white"
                      : "bg-violet-900/40 border-violet-700/50 text-violet-200 hover:border-fuchsia-500/50 hover:text-white"
                  }`}
                >
          ...
          <button
            ...
            disabled={!answered}
            className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-fuchsia-500 to-rose-500 text-white font-semibold disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02] transition-all shadow-lg shadow-fuchsia-500/20"
          >
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-violet-950 via-fuchsia-950 to-rose-950 pb-24">
      <div className="max-w-lg mx-auto px-4 pt-8">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-violet-400">Let&apos;s get started</span>
            <span className="text-xs text-violet-400">5%</span>
          </div>
          <div className="h-1.5 rounded-full bg-violet-800/60">
            <div className="h-full rounded-full bg-gradient-to-r from-fuchsia-500 to-rose-500 transition-all duration-500" style={{ width: "5%" }} />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Friendr Quiz</h1>
            <p className="text-violet-300/80 text-sm">A few questions to find your people</p>
          </div>
        </div>
          ...
          {(["male", "female", "any"] as const).map((opt) => (
            <button
              ...
              className={`w-full text-left px-5 py-4 rounded-xl border text-sm font-medium transition-all ${
                companion === opt
                  ? "bg-fuchsia-500/25 border-fuchsia-500/70 text-white"
                  : "border-violet-600/60 text-violet-300 hover:border-fuchsia-500/60 hover:text-white bg-violet-900/40"
              }`}
            >
              <div className="flex items-center justify-between">
                <span>{opt === "male" ? "Male" : opt === "female" ? "Female" : "Any / Open to anyone"}</span>
                {companion === opt && <Check className="w-4 h-4 text-fuchsia-400" />}
              </div>
            </button>
          ))}
        </div>
